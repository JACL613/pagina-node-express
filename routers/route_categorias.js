const Categorias = require("../Databases/models/categorias_schema");
...
route.get("/:id", async (req, res) => {
  ...
  try {
    const query = await Categorias.findById(id);
    if (!query)
      return res.json({
        status: 404,
...
route.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    const query = await Categorias.create({ name });
    if (!query)
...
route.put("/:id", authMiddleware, async (req, res) => {
  ...
  if (!id || !data) return res.json({ message: "faltan datos", status: 401 });
    try {
        const query = await Categorias.findByIdAndUpdate(id, {...data})
...
route.delete("/:id", authMiddleware, async (req, res) => {
    ...
    if (!id) return res.json({ message: "faltan datos", status: 401 });
    try {
        const query = await Categorias.findByIdAndDelete(id)
